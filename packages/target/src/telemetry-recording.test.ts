...
describe("TelemetryRecorder", () => {
  ...
  it("counts physical acquisition sequence gaps in the recording metadata", () => {
    const recorder = new TelemetryRecorder();
    recorder.start();
    recorder.capture({ ...sample(2), source: "physical" });
    recorder.capture({ ...sample(3), source: "physical" });
    recorder.capture({ ...sample(7), source: "physical" });

    expect(recorder.stop().droppedSamples).toBe(3);
  });

  it("does not infer lost observations from legacy virtual physics-step gaps", () => {
    const recorder = new TelemetryRecorder();
    recorder.start();
    recorder.capture(sample(2));
    recorder.capture(sample(300));
    expect(recorder.stop().droppedSamples).toBe(0);
  });

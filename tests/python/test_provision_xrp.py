...
class ProvisionXrpTest(unittest.TestCase):
    def test_one_command_path_uses_retrying_usb_operations(self):
        ...
        self.assertEqual(result["courseRelease"], "2026.08-dev.23")
    ...
    def test_default_hotspot_needs_no_station_credentials_or_lan_reply(self):
        ...
        self.assertEqual(result["serviceVersion"], "0.1.0")
        self.assertFalse(result["serviceVerified"])
        self.assertEqual(result["releaseIdentitySource"], "local release files")
        self.assertIn("not network-verified", result["serviceVerification"])

    def test_local_service_version_reads_the_release_authority(self):
        self.assertEqual(PROVISION.local_service_version(), "0.1.0")

...
class TelemetryBufferTest(unittest.TestCase):
    ...
    def test_retains_a_bounded_ordered_set_of_immutable_step_snapshots(self):
        ticks = iter(range(1_000, 3_020, 20))
        ...
        self.assertEqual([item["sampleSeq"] for item in retained], list(range(6, 102)))
        self.assertEqual(
            [item["sampleTimeMs"] for item in retained],
            list(range(100, 2_020, 20)),
        )
        self.assertEqual(
            [item["sampleSeq"] for item in _telemetry.buffered_state_snapshots(98)],
            [99, 100, 101],
        )

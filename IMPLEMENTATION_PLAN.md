...
2. Implement the smallest reviewed API needed for the first experiments:
   ...
   conversion, wheel-speed control, differential-drive conversion, odometry,
   and the orchestration needed by Challenge 1. Do not replicate the draft
   blindly.
...
4. Build reference `.mpy` modules from one reference source tree using the
   ...
   Produce target-specific bytecode only if measured ABI evidence requires it.
...
3. Provide a downloadable recovery bundle for manual use.

...
class BufferedRequestReader:
    """Read socket chunks while preserving HTTP line and body boundaries.

    MicroPython's Stream.read yields to the socket poller on every call, even
    when more bytes are already available. Reading headers one byte at a time
    therefore delays every command by hundreds of event-loop turns.
    """
    ...
    async def read(self, count):
        if self.buffer:
            ...
            return value
        return await self.reader.read(min(count, READ_CHUNK_BYTES))
...
class Admission:
    def __init__(self, server, wait_for=None, on_error=None):
        self.server = server
    ...
    async def line(self, reader, limit=MAX_LINE_BYTES):
        return await reader.line(limit)
    ...
    def internal_error(self, error, request, phase):
        failure = HttpError(500, "internal_error", "The XRP service could not complete this operation")
        failure.fault_id = None
        if self.on_error is not None:
            try:
                failure.fault_id = self.on_error(error, request, phase)
            except Exception:
                pass  # Diagnostic failures must not hide the HTTP failure.
        return failure
    ...
    def dispatch(self, request):
        try:
            ...
            if error.status < 500:
                raise
            raise self.internal_error(error, request, "http_handler")
        except Exception as error:
            # An OSError raised by a route (for example, an I2C failure) is an
            # internal error, not evidence that the browser closed its socket.
            raise self.internal_error(error, request, "http_handler")

    async def send(self, writer, response, keep_alive=False, request=None):
        try:
            body = response.body
            if isinstance(body, str):
                body = body.encode("utf-8")
            if not isinstance(body, (bytes, bytearray)):
                raise TypeError("The course service returned an invalid response body")
            ...
            raise self.internal_error(error, request, "http_response")
        # Finish response serialization before writing any bytes. A malformed
        # response can then receive one complete 500, rather than two headers.
        writer.write(head)
        writer.write(body)
    ...
    def error_response(self, error):
        ...
        fault_id = getattr(error, "fault_id", None)
        if fault_id:
            detail["faultId"] = fault_id
    ...
    async def handle(self, reader, writer):
        reader = BufferedRequestReader(reader)
        request = None
        ...
        try:
            ...
            for index in range(MAX_REQUESTS_PER_CONNECTION):
                ...
                await self.send(writer, response, keep_alive, request)
                if not keep_alive:
                    return
        except Exception as error:
            ...
            if isinstance(error, TimeoutError):
                ...
                if isinstance(error, MemoryError):
                    ...
                    error = self.internal_error(error, request, "http_admission")
            try:
...
def install(server, on_error=None):
    ...
    admission = Admission(server, on_error=on_error)

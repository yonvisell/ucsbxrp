...
def run_challenge():
    ...
    if grid.column_count * grid.row_count > MAXIMUM_GRID_CELLS:
        raise ValueError("Use at most 1024 grid cells for this checkoff")
    ...
    if not EXECUTE_ROUTE:
        print("Motion-free checkoff complete. Set EXECUTE_ROUTE=True only when ready to follow the route.")
        return path
    ...
    try:
        ...
        while not navigation.is_complete():
            if elapsed_time_s(state.measurements.time_ms, start_ms) >= 120.0:
                print("Mapped Route: result=timeout")
                return state
